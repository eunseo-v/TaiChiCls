model = dict(
    ...
    backbone = dict(
        ...
        pretrained = None,
        in_channels = 34, # 采用5Part channel num of input features
...
train_pipeline = [
    ...
    dict(type = 'GeneratePoseTarget', sigma =0.6, use_score = True, with_kp = True, with_limb = True), # 训练集的double一定要是False! kp和limb在通道维上连接
...
val_pipeline = [
    ...
    dict(type = 'GeneratePoseTarget', sigma=0.6, use_score = True, with_kp = True, with_limb = True),
...
test_pipeline = [
    ...
    dict(type = 'Resize', scale=(-1, 64)),
    dict(type = 'CenterCrop', crop_size = 64),
    dict(type = 'GeneratePoseTarget', sigma = 0.6, use_score = True, with_kp=True, with_limb=True, double=True, left_kp= left, right_kp=right),
    dict(type = 'FormatShape', input_format = 'NCTHW'),
    dict(type = 'Collect', keys=['imgs', 'label'], meta_keys=[]),
    dict(type = 'ToTensor', keys=['imgs'])
]
