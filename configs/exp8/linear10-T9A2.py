model = dict(
    ...
    backbone = dict(
        ...
        pretrained = None,
        in_channels = 10, # 采用5Part channel num of input features
        ...
        frozen_stages = 3 # stem layer 和 resnet layer都冻结权重
    ),
    cls_head = dict(
        type = 'I3DHead',
        in_channels = 512, 
...
ann_file_train = 'data/ds_taichi/TEST9AUG2/NSNR/train_data.pkl'
ann_file_val = 'data/ds_taichi/TEST9AUG2/NSNR/test_data.pkl'
left = [
...
train_pipeline = [
    ...
    dict(type = 'GenerateTaiChiPoseTarget', sigma =0.6, use_score = True, with_kp = True, with_limb = True), # 训练集的double一定要是False!
...
val_pipeline = [
    ...
    dict(type = 'GenerateTaiChiPoseTarget', sigma=0.6, use_score = True, with_kp = True, with_limb = True),
...
test_pipeline = [
    ...
    dict(type = 'Resize', scale=(-1, 64)),
    dict(type = 'CenterCrop', crop_size = 64),
    dict(type = 'GenerateTaiChiPoseTarget', sigma = 0.6, use_score = True, with_kp = True, with_limb = True, double=True, left_kp= left, right_kp=right),
    dict(type = 'FormatShape', input_format = 'NCTHW'),
    dict(type = 'Collect', keys=['imgs', 'label'], meta_keys=[]),
    dict(type = 'ToTensor', keys=['imgs'])
]
...
'''
output_config = dict(
    out = './model_pth/exp8/linear10-T9A2/test_result/results.pkl'
) # 保存测试集的各类分类概率和样本名 keys:'outputs','sample_names'
eval_config = dict(
    metric_out = './model_pth/exp8/linear10-T9A2/test_result',
    eval = ['top_k_accuracy', 'mean_class_accuracy', 'confusion_matrix', 't_sne_vis'],
) # train中不写test_last test_best 在test.py中测试
dist_params = dict(backend='nccl')
log_level = 'INFO'
work_dir = './model_pth/exp8/linear10-T9A2'
load_from = './model_pth/10part_ntu60_xsub_kp+limb/best_top1_acc_epoch_24.pth'
resume_from = None
