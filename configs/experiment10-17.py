...
train_pipeline = [
    ...
    dict(type = 'GeneratePoseTarget', sigma =0.6, use_score = True, with_kp = True, with_limb = False), # 训练集的double一定要是False!
...
test_pipeline = [
    ...
    dict(type = 'Resize', scale=(-1, 64)),
    dict(type = 'CenterCrop', crop_size = 64),
    dict(type = 'GeneratePoseTarget', sigma = 0.6, use_score = True, with_kp=True, with_limb=False, double=True, left_kp= left, right_kp=right),
    dict(type = 'FormatShape', input_format = 'NCTHW'),
    dict(type = 'Collect', keys=['imgs', 'label'], meta_keys=[]),
    dict(type = 'ToTensor', keys=['imgs'])
]
...
'''
output_config = dict(
    out = './model_pth/17part_ntu60_xsub_kp_0511/test_result/results.pkl'
)
eval_config = dict(
    metric_out = './model_pth/17part_ntu60_xsub_kp_0511/test_result',
    eval = ['top_k_accuracy', 'mean_class_accuracy', 'confusion_matrix'],
) # train中不写test_last test_best 在test.py中测试
dist_params = dict(backend='nccl')
log_level = 'INFO'
work_dir = './model_pth/17part_ntu60_xsub_kp_0511'
load_from = './model_pth/exp9/T3-fintune-with-pre-lrd01/epoch_24.pth'
resume_from = None
