...
'''
output_config = dict(
    out = './model_pth/exp5/fintune-without-pre/test_result/results.pkl'
) # 保存测试集的各类分类概率和样本名 keys:'outputs','sample_names'
eval_config = dict(
    metric_out = './model_pth/exp5/fintune-without-pre/test_result',
    eval = ['top_k_accuracy', 'mean_class_accuracy', 'confusion_matrix', 't_sne_vis'],
) # train中不写test_last test_best 在test.py中测试
dist_params = dict(backend='nccl')
log_level = 'INFO'
work_dir = './model_pth/exp5/fintune-without-pre'
load_from = None
resume_from = None
