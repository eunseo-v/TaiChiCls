...
train_pipeline = [
    ...
    dict(type = 'GenerateNTUPoseTarget', sigma =0.6, use_score = True, with_kp = False, with_limb = True), # 训练集的double一定要是False!
...
val_pipeline = [
    ...
    dict(type = 'GenerateNTUPoseTarget', sigma=0.6, use_score = True, with_kp = False, with_limb = True),
...
test_pipeline = [
    ...
    dict(type = 'Resize', scale=(-1, 64)),
    dict(type = 'CenterCrop', crop_size = 64),
    dict(type = 'GenerateNTUPoseTarget', sigma = 0.6, use_score = True, with_kp=False, with_limb=True, double=True, left_kp= left, right_kp=right),
    dict(type = 'FormatShape', input_format = 'NCTHW'),
    dict(type = 'Collect', keys=['imgs', 'label'], meta_keys=[]),
    dict(type = 'ToTensor', keys=['imgs'])
]
...
log_level = 'INFO'
work_dir = './model_pth/5part_ntu60_xsub_limb_0412'
load_from = None
resume_from = None
