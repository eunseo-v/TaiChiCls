model = dict(
    ...
    backbone = dict(
        ...
        pretrained = None,
        in_channels = 17, # 采用5Part channel num of input features
...
train_pipeline = [
    ...
    dict(type = 'GenerateTaiChi17PoseTarget', sigma =0.6, use_score = True, with_kp = True, with_limb = False), # 训练集的double一定要是False!
...
val_pipeline = [
    ...
    dict(type = 'GenerateTaiChi17PoseTarget', sigma=0.6, use_score = True, with_kp = True, with_limb = False),
...
test_pipeline = [
    ...
    dict(type = 'Resize', scale=(-1, 64)),
    dict(type = 'CenterCrop', crop_size = 64),
    dict(type = 'GenerateTaiChi17PoseTarget', sigma = 0.6, use_score = True, with_kp=True, with_limb=False, double=True, left_kp= left, right_kp=right),
    dict(type = 'FormatShape', input_format = 'NCTHW'),
    dict(type = 'Collect', keys=['imgs', 'label'], meta_keys=[]),
    dict(type = 'ToTensor', keys=['imgs'])
]
...
'''
output_config = dict(
    out = './model_pth/exp2/NSNR-linear17/test_result/results.pkl'
)
eval_config = dict(
    metric_out = './model_pth/exp2/NSNR-linear17/test_result',
    eval = ['top_k_accuracy', 'mean_class_accuracy', 'confusion_matrix', 't_sne_vis'],
) # train中不写test_last test_best 在test.py中测试
dist_params = dict(backend='nccl')
log_level = 'INFO'
work_dir = './model_pth/exp2/NSNR-linear17'
