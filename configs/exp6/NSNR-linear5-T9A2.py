...
train_pipeline = [
    ...
    dict(type = 'GenerateTaiChiPoseTarget', sigma =0.6, use_score = True, with_kp = False, with_limb = True), # 训练集的double一定要是False!
...
test_pipeline = [
    ...
    dict(type = 'Resize', scale=(-1, 64)),
    dict(type = 'CenterCrop', crop_size = 64),
    dict(type = 'GenerateTaiChiPoseTarget', sigma = 0.6, use_score = True, with_kp = False, with_limb = True, double=True, left_kp= left, right_kp=right),
    dict(type = 'FormatShape', input_format = 'NCTHW'),
    dict(type = 'Collect', keys=['imgs', 'label'], meta_keys=[]),
    dict(type = 'ToTensor', keys=['imgs'])
]
...
'''
output_config = dict(
    out = './model_pth/exp6/NSNR-linear5-T9A2-limb/test_result/results.pkl'
) # 保存测试集的各类分类概率和样本名 keys:'outputs','sample_names'
eval_config = dict(
    metric_out = './model_pth/exp6/NSNR-linear5-T9A2-limb/test_result',
    eval = ['top_k_accuracy', 'mean_class_accuracy', 'confusion_matrix', 't_sne_vis'],
) # train中不写test_last test_best 在test.py中测试
dist_params = dict(backend='nccl')
log_level = 'INFO'
work_dir = './model_pth/exp6/NSNR-linear5-T9A2-limb'
load_from = './model_pth/5part_ntu60_xsub_limb_0412/best_top1_acc_epoch_24.pth'
resume_from = None
