...
mmdet_root = '~/mmaction2/mmdetection' # ~指用户根目录
...
def my_pose_extraction(vid, skip_postproc=False):
    frame_paths = extract_frame(vid)
    det_results = detection_inference(args, frame_paths)
    if not skip_postproc:
        det_results = ntu_det_postproc(vid, det_results)
...
def parse_args():
    ...
    parser.add_argument('--video', type=str, default='wjsn_dance.mp4', help='source video')
    parser.add_argument('--output', type=str, default='wjsn_dance.pkl', help='output pickle name')
    parser.add_argument('--device', type=str, default='cuda:0')
    parser.add_argument('--skip-postproc', action='store_true', default='False')
...
if __name__ == '__main__':
    ...
    anno = my_pose_extraction(args.video, True)
