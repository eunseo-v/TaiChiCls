...
def taichi_shear_rot_aug(data, shear = False, rotate = False):
    '''
        Args:
            data [T, V, C]
            rotate 绕y轴方向随机旋转±60°
    ...
    '''
    if rotate == True:
        rotate_angle = random.uniform(-60/180*np.pi, 60/180*np.pi)
    ...
    if shear == True:
        sap = 0.5 # shear amplitude
        s1_list = [random.uniform(-sap, sap), random.uniform(-sap, sap), random.uniform(-sap, sap)]
        s2_list = [random.uniform(-sap, sap), random.uniform(-sap, sap), random.uniform(-sap, sap)]
...
def taichi_skeleton_annotation(skeleton, shear = False, rotate = False, train_mode = False):
    '''
    ...
    if train_mode == True:
        data = taichi_shear_rot_aug(data, shear=shear, rotate=rotate)
    else: # 测试集保留原数据形式
        data = data
        # data [T, V, C]
...
def taichi_aug_method(random_seed, shear = False, rotate = False):
    ...
    for skeleton in train_aug_sample:
...
if __name__ == '__main__':
    ...
    taichi_aug_method(random_seed=42, shear=True, rotate=True) # WSWR
    taichi_aug_method(random_seed=42, shear=True, rotate=False) # WSNR
    taichi_aug_method(random_seed=42, shear=False, rotate=True) # NSWR
    taichi_aug_method(random_seed=42, shear=False, rotate=False) # NSNR